...
urlpatterns = [
    ...
    url(
        r"^selecttwo/(?P<app_name>[a-z]+)/(?P<model_name>[a-z]+)/(?P<filter_field>[a-z]+)/(?P<filter_param>[a-z]+)/",
        selecttwo,
    ),
    url(
        r"^chosen/(?P<app_name>[a-z]+)/(?P<model_name>[a-z]+)/(?P<filter_field>[a-z]+)/(?P<filter_param>[a-z]+)/",
        chosen,
    ),
    url(
        r"^selecttwo_create/(?P<app_name>[a-z]+)/(?P<model_name>[a-z]+)/(?P<filter_field>[a-z]+)/(?P<filter_param>[a-z]+)/",
        selecttwo_create,
    ),
    url(
        r"^thumbnail/(?P<app_name>[a-z]+)/(?P<model_name>[a-z]+)/(?P<pk>[0-9]+).jpg", th
    ),
    url(
        r"^thumbnail/(?P<app_name>[a-z]+)/(?P<model_name>[a-z]+)/(?P<pk>[0-9]+)/(?P<res>[0-9]+).jpg",
        th,
    ),
