...
def projectplace(request, project_pk):
    pcodes = [
        p.place.pcode for p in Project.objects.get(pk=project_pk).projectplace_set.all()
    ]
...
def projectplace_intersect_convex_hull(request, project_pk):
    ...
    AdminArea.objects.filter(pcode__in=places_list).aggregate(Union("geom"))[
        "geom__union"
    ].convex_hull
...
def placeconvexhulls(request):
    ...
    for a in AdminArea.objects.all():
        returns[a.pcode] = [
            (round(i[1], 3), round(i[0], 3))
            for i in a.geom.convex_hull.simplify(0.005).coords[0]
        ]
