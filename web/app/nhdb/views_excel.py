...
def project(request, object_list=None):
    ...
    object_list = object_list.prefetch_related(
        "projectorganization_set", "organization"
    )
    ...
    for obj in object_list:
        ...
        organizations_primary = ", ".join(
            [
                i.organization.name
                for i in obj.projectorganization_set.filter(
    ...
    return workbook.response(
        filename="nhdb-projects-{}.xlsx".format(date.today().isoformat())
    )
...
def organization(request, object_list=None):
    ...
    object_list = (
        object_list.prefetch_related(
            "organizationplace_set__organizationplacedescription"
        )
    ...
    for object in object_list:
        ...
        if object.primary_contact_person:
            sheet.write(
                row, 4, u"{}".format(object.primary_contact_person.first() or "")
            )
        ...
        for organizationplace in object.organizationplace_set.all():
            ...
            try:
                sheet.write(
                    row + address_row,
                    6,
                    organizationplace.organizationplacedescription.suco,
                )
                sheet.write(
                    row + address_row,
                    7,
                    organizationplace.organizationplacedescription.subdistrict,
                )
                sheet.write(
                    row + address_row,
                    8,
                    organizationplace.organizationplacedescription.district,
                )
            except:
                pass
            address_row += 1
    ...
    return workbook.response(
        filename="nhdb-organizations-{}.xlsx".format(date.today().isoformat())
    )
