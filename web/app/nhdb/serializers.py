...
class ProjectSerializer(serializers.ModelSerializer):
    class Meta:
        model = Project
        fields = (
            'id', 'name', 'description', 'startdate', 'enddate', 'verified', 'status',
...
class ProjectPersonSerializer(SuggestedChangeSerializer):
    ...
    class Meta:
        model = ProjectPerson
        fields = ('id', 'project', 'person', 'is_primary', 'verified', 'project_title', 'person_name')
...
class ProjectPlaceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProjectPlace
        fields = ('id', 'description', 'project', 'place', 'project_title', 'place_name')
    ...
    def update(self, instance, validated_data):
        ...
        for p in projectplaces:
            ...
            if p.get('id'):
                ...
                pp = ProjectPlace.objects.get_or_create(project=instance, place=p.get('place'))[0]
    ...
    def update(self, instance, validated_data):
        ...
        for p in projectplaces:
            ...
            if p.get('id'):
                ...
                pp = ProjectPerson.objects.get_or_create(project=instance, person=p.get('person'))[0]
...
class ProjectProjectOrganizationSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        for p in projectorganizations:
            po = ProjectOrganization.objects.get_or_create(project=instance, organization=p.get('organization'))
...
class OrganizationOrganizationPlaceSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Drop out any id's which have been removed
        dropped_places = instance.organizationplace_set.exclude(pk__in=organizationplace_ids)
        dropped_places.delete()
