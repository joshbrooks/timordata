...
def filter_from_params(req, objects, targetfields=None):
    ...
    if targetfields == None:
        targetfields = {
            "ben": {"field": "targetbeneficiaries__id", "table": TargetBeneficiaries},
            "org": {
                "field": "project_organization__organization_id",
                "table": Organization,
            },
            "inv": {"field": "involvementareas__id", "table": AreaOfInvolvement},
            "act": {"field": "mainactivities__id", "table": MainActivity},
    ...
    def districthandler(objects):
        ...
        for d in _g(param):
            ...
            filtering.append(
                {
                    "class": "filtertext include",
                    "param": "x_" + param,
                    "name": "Filter to include only where {0[table]} is {0[name]}".format(
                        opts
                    ),
                }
            )
        ...
        for d in _g("x_" + param):
            ...
            filtering.append(
                {
                    "class": "filtertext exclude",
                    "param": "x_" + param,
                    "name": "Filter to exclude where {0[table]} is {0[name]}".format(
                        opts
                    ),
                }
            )
    ...
    def standardpkhandler(objects):
        ...
        for param in targetfields.keys():
            ...
            if param_name["field"].endswith("_id"):
                for value in _g(param):
                    ...
                    print(newq)
                    opts = {
                        "name": param_name["table"].objects.get(pk=value),
                        "table": param_name["table"]
                        .objects.get(pk=value)
                        ._meta.verbose_name.capitalize(),
                    }
                    _filtering.append(
                        {
                            "class": "filtertext include",
                            "param": param,
                            "name": "Filter to only include where {0[table]} is {0[name]}".format(
                                opts
                            ),
                        }
                    )
        ...
        for param in targetfields.keys():
            ...
            if param_name["field"].endswith("_id"):
                if _g("x_" + param) != []:
                    ...
                    if "x_" + param not in req.GET.keys():
                        continue
    ...
    def orgtypefilter(objects):
        ...
        for value in _g("otype"):
            ...
            _filtering.append(
                {
                    "class": "filtertext include",
                    "name": "Filter to only include where {0[table]} is an {0[name]}".format(
                        opts
                    ),
                }
            )
        ...
        for value in _g("x_otype"):
            ...
            _filtering.append(
                {
                    "class": "filtertext exclude",
                    "name": "Filter to only include where {0[table]} is not an {0[name]}".format(
                        opts
                    ),
                }
            )
    ...
    def orgnamelike(objects):
        ...
        for value in _g("orgname"):
            ...
            _filtering.append(
                {
                    "class": "filtertext include",
                    "name": "Filter to only include where {0[table]} name is like {0[name]}".format(
                        opts
                    ),
                }
            )
