...
class ProjectManager(models.Manager):
    ...
    def active(self, include_unknown=False):
        ...
        return (
            super(ProjectManager, self).get_queryset().filter(status__pk="A").filter(q)
        )

    def inactive(self):
        return super(ProjectManager, self).get_queryset().exclude(status__pk="A")
...
class OrganizationManager(models.Manager):
    ...
    def no_email(self):
        return (
            super(OrganizationManager, self).get_queryset().filter(email__isnull=True)
        )
...
class ExcelDownloadFeedback(models.Model):
    ...
    purposeother = models.CharField(
        _("Other purpose"), max_length=150, null=True, blank=True
    )
...
class Organization(models.Model):
    ...
    orgtype = models.ForeignKey(
        "OrganizationClass", verbose_name=_("class"), default="LNGO"
    )
    active = models.BooleanField(default=True)
    fongtilid = models.IntegerField(
        null=True, blank=True, verbose_name="Org. ID (Fongtil)"
    )
    justiceid = models.IntegerField(
        null=True, blank=True, verbose_name="Org. ID (Min. Justice)"
    )
    stafffulltime = models.IntegerField(
        null=True, blank=True, verbose_name="Full time staff"
    )
    staffparttime = models.IntegerField(
        null=True, blank=True, verbose_name="Part time staff"
    )
    verified = models.DateField(null=True, blank=True, auto_now=True)
    phoneprimary = models.CharField(
        max_length=64, null=True, blank=True, verbose_name="Phone"
    )
    phonesecondary = models.CharField(
        max_length=64, null=True, blank=True, verbose_name="Alternate phone"
    )
    ...
    def filecounts(self):
        ...
        for lang in settings.LANGUAGES_FIX_ID:
            ...
            count = (
                version_model.objects.filter(publication__organization=self)
                .exclude(**kw)
                .count()
            )
            if count > 0:
...
class Person(models.Model):
    ...
    primary_contact_of = models.ForeignKey(
        "nhdb.Organization",
        null=True,
        blank=True,
...
class ProjectImage(models.Model):
    def __str__(self):
        try:
            if self.project:
                return u"Image from project {} - {}".format(
...
class Project(models.Model):
    ...
    status = models.ForeignKey(
        "ProjectStatus", null=True, verbose_name="Status", default="A"
    )
    ...
    beneficiary = models.ManyToManyField(
        "nhdb.PropertyTag",
        blank=True,
        related_name="project_beneficiary",
        limit_choices_to={"path__startswith": "BEN."},
    )

    place = models.ManyToManyField("geo.AdminArea", through="ProjectPlace", blank=True)
    organization = models.ManyToManyField(
        Organization, through="ProjectOrganization", blank=True
    )
    stafffulltime = models.IntegerField(
        null=True, blank=True, verbose_name=_("Full time staff")
    )
    staffparttime = models.IntegerField(
        null=True, blank=True, verbose_name=_("Part time staff")
    )
    ...
    def feature(self):
        ...
        properties = {"name": self.name, "id": self.pk}

        f = {
            "type": "Feature",
            "geometry": {"type": "MultiPolygon", "coordinates": self.coordinates},
...
class ProjectOrganization(models.Model):
    ...
    organizationclass = models.ForeignKey(
        "ProjectOrganizationClass",
        default="P",
    ...
    notes = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name=_("Notes about this relationship"),
    )
...
class ProjectPlace(models.Model):
    def __str__(self):
        if self.description:
            ...
            return u"{}, {}".format(self.project.name, self.place.name)
        else:
            return "?"
    ...
    def feature(self):
        ...
        properties = {"description": self.description, "id": self.pk}
...
class OrganizationPlace(models.Model):
    ...
    def update_place(self):
        ...
        description, created = OrganizationPlaceDescription.objects.get_or_create(
            organizationplace=self
        )
        try:
