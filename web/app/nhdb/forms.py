...
def getchoices(model, field="pk", field_display=None):
    try:
        return [
            (getattr(i, field), getattr(i, field_display)) for i in model.objects.all()
        ]
    except:
        # raise a warning
        return []
    ...
    def helper(self):
        ...
        helper.layout.extend(
            [
                ...
                Fieldset("Size", "stafffulltime", "staffparttime"),
            ]
        )
...
class OrganizationDescriptionForm(SuggestionForm):
    ...
    def __init__(self, language="en", suggest=None, organization=None, *args, **kwargs):
        ...
        self.suggest, self.organization, self._language = (
            suggest,
            organization,
            language,
        )
...
def get_adminarea_list():
    ...
    try:
        for place in (
            apps.get_model("geo", "adminarea")
            .objects.order_by("path")
            .values_list("pk", "name")
        ):
            placechoices.append(
                (place[0], mark_safe(u"{}{}".format(indent(place[1]), place[1])))
            )
    except:
        pass
    return placechoices
...
class OrganizationSearchForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["act"].choices = [
            (p.path, _(p.name))
            for p in PropertyTag.objects.filter(path__startswith="ACT.")
        ]
        self.fields["ben"].choices = [
            (p.path, _(p.name))
            for p in PropertyTag.objects.filter(path__startswith="BEN.")
        ]
        self.fields["inv"].choices = [
            (p.path, _(p.name))
            for p in PropertyTag.objects.filter(path__startswith="INV.")
        ]
        self.fields["orgtype"].choices = [
            (i.pk, i.orgtype) for i in OrganizationClass.objects.all()
        ]
            ...
            Div(
                Fieldset(
                    ...
                    Field(
                        "act",
                        data_selecttwo="true",
                        css_class="select2_auto",
                        wrapper_class="collapse",
                    ),
                    Field(
                        "inv",
                        data_selecttwo="true",
...
class ProjectpropertiesForm(SuggestionForm):
    ...
    def helper(self):
        if isinstance(self._instance, Project):
            helper = self.get_helper(
                url="/rest/nhdb/projectpropertiesbyid/{}/".format(self._instance.pk)
            )
        elif isinstance(self._instance, Suggest):
            helper = self.get_helper(
                url="/rest/nhdb/projectpropertiesbyid/_{}/".format(self._instance.pk)
            )
        ...
        helper.layout.extend(
            [
                ...
                FormActions(Submit("__action", "Update")),
            ]
        )
...
class ProjectTranslationsForm(SuggestionForm):
    ...
    def helper(self):
        helper = self.get_helper()
        a = [
            AccordionGroup(n, "name_" + c, "description_" + c)
            for c, n in LANGUAGES_FIX_ID
        ]
...
class ProjectOtherPropertiesForm(SuggestionForm):
    ...
    def __init__(self, project, *args, **kwargs):
        ...
        super(ProjectOtherPropertiesForm, self).__init__(
        ...
        for field_name in field_list:
            ...
            if instance:
                fields.append(
                    Field(
                        field_name_trans,
                        value=getattr(instance, field_name_trans, "") or "",
                    )
                )
            else:
...
class ProjectForm(SuggestionForm):
    ...
    def helper(self):
        ...
        helper.layout.append(
            translation_accordion(
                field_list=["name", "description"], instance=self.project
            )
        )
    ...
    def helper(self):
        ...
        helper.layout.extend(
            [
                ...
                Field(
                    "project",
                    data_selecturl="/selecttwo/nhdb/project/name/icontains",
...
class BaseSuggestionForm(forms.Form):
    ...
    comment = forms.CharField(
        widget=forms.TextInput(), label="Comment on your changes", required=False
    )
...
class ProjectorganizationForm(SuggestionForm):
    ...
    def __init__(
        self, projectorganization=None, project=None, organization=None, *args, **kwargs
    ):

        super(ProjectorganizationForm, self).__init__(
            projectorganization, *args, **kwargs
        )
        ...
        self.set_field_opts(
            name=["project", "organization"], instance=projectorganization
        )
...
class ProjectSearchForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["inv"] = forms.MultipleChoiceField(
            ...
            label=_("Sector(s)"),
            choices=[
                (i.lowerpathstring(), i.name)
                for i in PropertyTag.objects.filter(path__startswith="INV.")
            ],
        )
        self.fields["ben"] = forms.MultipleChoiceField(
            ...
            choices=[
                (i.lowerpathstring(), i.name)
                for i in PropertyTag.objects.filter(path__startswith="BEN.")
            ],
        )
        self.fields["act"] = forms.MultipleChoiceField(
            ...
            choices=[
                (i.lowerpathstring(), i.name)
                for i in PropertyTag.objects.filter(path__startswith="ACT.")
            ],
        )

    name = forms.CharField(required=False, max_length=100, label="Search text")
    ...
    organization = forms.TypedChoiceField(required=False)
    ...
    startdateafter = forms.DateField(required=False, label="Starts after date")
...
class OrganizationplaceForm(SuggestionForm):
    ...
    def __init__(self, organization=None, organizationplace=None, *args, **kwargs):
        ...
        if organizationplace and organizationplace.point:
            self.point_field_value = "{},{}".format(
                self.instance.point.x, self.instance.point.y
            )
        else:
            self.point_field_value = ""

        if organization is None and organizationplace is not None:
            if hasattr(organizationplace, "organization") and isinstance(
                organizationplace.organization, Organization
            ):
    ...
    def helper(self):
        ...
        """.format(
            self.point_field_value, self.point_field_value
        )
...
class CollapsibleTranslationFieldset:
    ...
    def __init__(
        ...
        default_fields=("path", "name", "description"),
    ...
    def fieldsets(self, collapsible=True, css_class="row col-md-4"):
        ...
        returns = []
        fields = [
            Field("%s" % (i), wrapper_class=self.wrapper_class)
            for i in self.default_fields
        ]
        ...
        for language in self.languages:
            ...
            fields = [
                Field("%s_%s" % (i, language[0]), wrapper_class=self.wrapper_class)
...
class ProjectPersonForm(SuggestionForm):
    ...
    def helper(self):
        ...
        helper.layout.append(
            Field(
                "person",
                data_selecturl="/selecttwo/nhdb/person/name/icontains",
                **create_person_elements
            )
        )
    ...
    def __init__(self, person=None, *args, **kwargs):
        ...
        if self.instance.organization:
            self.fields["organization"].choices = [
                (self.instance.organization.pk, self.instance.organization)
            ]
        else:
            self.fields["organization"].choices = []
        self.person = person
...
class ProjectPlaceForm(SuggestionForm):
    ...
    def helper(self):
        ...
        helper.layout.append(
            Field("place", data_selecturl="/selecttwo/geo/adminarea/name/icontains")
        )
