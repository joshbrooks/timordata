...
def _d(string):
    ...
    if string is None or string == "":
        ...
        try:
            ...
            logging.warn(
                "Ran into a string which could not be handled - {}".format(string)
            )
...
def mysql_timestamp_to_date(timestamp):
    ...
    if "/" in datestring:
        ...
        logging.warn(
            "Ran into a timestamp which could not be handled - {} has no separator. Blank date returned.".format(
                timestamp
            )
        )
        return None
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logging.warn(
                    "{} Ran into a timestamp which could not be handled - {}. Blank date returned.".format(
                        e, timestamp
                    )
                )
                return None
...
def setup():
    ...
    for key, kw in m.projectorganizationclass.items():
        pc, created = ProjectOrganizationClass.objects.get_or_create(
            code=kw.get("code")
        )
        if created:
    ...
    while row is not None:
        ...
        try:
            organization, created = Organization.objects.get_or_create(
                pk=row.get("pkOrganizationId"), orgtype=orgtype
            )
...
def importpersons():
    ...
    c.execute(
        """SELECT pkPersonId, firstName, lastName, concat(firstName,' ',lastName), title, fkOrganizationId, phone, mobilePhone, email, verified from tblpeople WHERE fkOrganizationId IS NOT NULL AND firstName IS NOT NULL;"""
    )
    ...
    for (
        ...
        try:
            ...
            logging.warn(
                "Experienced an error creating Person /n {}".format(
                    (pk, first, last, name, title, org_id)
                )
            )
            logging.warn(e)
...
def projectproperties():
    ...
    for pid, benid in c.fetchall():
        try:
            ...
            if not d:
                logging.warn(
                    "Specified {} pk={} which does not exist".format(
                        "beneficiary", benid
                    )
                )
                continue
    ...
    for pid, actid in c.fetchall():
        try:
            ...
            if not d:
                logging.warn(
                    "Specified {} pk={} which does not exist".format("activity", actid)
                )
                continue
            tag = PropertyTag.objects.get(path=d["path"])
    ...
    for project_id, person_id, primary in c.fetchall():
        ...
        try:
            ProjectPerson.objects.get_or_create(
                project_id=project_id, person_id=person_id, is_primary=primary
            )
...
def projectaddresses():
    ...
    for pid, districtcode, subdcode, sucocode, desc in c.fetchall():
        try:
            ...
            continue
        try:
            placeObject = Place.objects.get(
                pk=coalesce((sucocode, subdcode, districtcode), m.pcodes)
            )
            ...
            if sucocode == 999999:
                ...
                logging.warn(
                    "Place ({} / {} / {}) does not exist".format(
                        sucocode, subdcode, districtcode
                    )
                )
            continue
...
def organizationplace():
    ...
    for oid, pcodea, pcodeb, pcodec, desc, phone, email in c.fetchall():
        ...
        try:
            ...
            logging.warn(
                "{} {} {}".format(
                    e, oid, "This organization needs its location updated"
                )
            )
            continue
        ...
        try:
            ...
            logging.warn(
                "{} {} {}".format(
                    e, oid, "This organization is not in the new database"
                )
            )
            continue
