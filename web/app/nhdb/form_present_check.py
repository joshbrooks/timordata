...
def form_present(my_models, my_forms):
    def writeform(outfile, rendered, form_class, model, prefix="", instance=None):
        ...
        try:
            ...
            rendered.write(
                '<div class="container"><div class="row"><div class="col col-lg-6"><h4>%s%sForm</h4>'
                % (prefix, model_name)
            )
            if instance:
    ...
    def writedeleteform(outfile, rendered, form_class, model, instance):
        return writeform(
            outfile, rendered, form_class, model, prefix="Delete", instance=instance
        )
    ...
    with open("/tmp/forms.py", "w") as outfile:
        with open("/tmp/auto_generated_forms.html", "w") as rendered:
            ...
            form_list = [
                i
                for i in dir(my_forms)
                if isinstance(getattr(my_forms, i), ModelFormMetaclass)
            ]
            print(form_list)
            model_list = [
                i
                for i in dir(my_models)
                if isinstance(getattr(my_models, i), ModelBase)
            ]
            print(model_list)
                ...
                for i in form_list:
                    if "{}deleteform".format(model_name).lower() != i.lower():
                        ...
                        try:
                            ...
                            writedeleteform(
                                outfile, rendered, form_class, model, instance
                            )
                        except:
                            raise
                        break
