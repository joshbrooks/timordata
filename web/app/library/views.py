...
def publicationlist(request):
    ...
    def object_list(request, prefetch=("versions", "pubtype", "organization")):
        ...
        for term in version_filter_terms:
            if GET.getlist(term):
                filters["versions__" + term + "__in"] = [
                    PropertyTag.separatestring(i).upper()
                    for i in request.GET.getlist(term)
                ]
        ...
        for language_id in language_ids:
            kw = {
                "versions__title_" + language_id + "__isnull": False,
    ...
    def dashboard(publications=None):
        ...
        publications = publications.prefetch_related(
            "versions__tag__name", "versions__sector__name", "org"
        )
    ...
    context = {
        ...
        "filters": {
            "language_id": [
                {"value": s[0], "label": s[1]} for s in settings.LANGUAGES_FIX_ID
            ],
            "tag__id": Tag.objects.all(),
            "sector__path": PropertyTag.objects.filter(path__startswith="INV."),
            "pubtype": [
                {"value": p.pk, "label": p.name}
                for p in Pubtype.objects.exclude(code="PRI")
            ],
            "org": [
                {"value": o.pk, "label": u"{}".format(o.name)}
                for o in Organization.objects.annotate(
                    num_publications=Count("publication")
                ).filter(num_publications__gt=0)
            ],
        },
        "activefilters": {},
    }
    ...
    for f in context["filters"].keys():
        ...
        context["activefilters"]["primarysource"] = (
            request.GET.get("primarysource") or "false"
        )
...
def version_thumbnail_nginx(request):
    ...
    if d == "publication_pages":
        ...
        )  # Cut off the last underscore + digit (page number)
        ...
        if not version:
            return HttpResponseNotFound(
                open("/webapps/project/media/404.jpg").read(), content_type="image/jpg"
            )
        ...
        t = version.page_image(
            language=lang,
            res=res,
            root="/webapps/project/media/publication_covers/",
            page="cover",
        )
        return HttpResponse(open(t).read(), content_type="image/jpg")
...
def version_thumbnail(request, version_pk, language="en"):
    ...
    """
    _g = request.GET.get
    kw = {"res": _g("res", 150), "_format": _g("format", "jpg")}
    ...
    try:
        ...
        return HttpResponse(
            open(thumbnail.thumbnailPath).read(), content_type="image/" + kw["_format"]
        )
...
def publicationdashboard(request):
    ...
    try:
        context["publication"] = Publication.objects.get(
            pk=request.GET.get("publication")
        )
    except:
...
def form(request, model, form="main"):
    ...
    if settings.DEBUG:
        raise NotImplementedError(
            "Class nhdb.forms.{} is not defined yet".format(f_name)
        )
...
def suggested_publications(request):
    ...
    context["suggestions"] = Suggest.objects.filter(
        affectedinstance__in=AffectedInstance.objects.filter(
            primary=True, model_name="library_publication"
        ),
        action="CM",
    )
