...
def version_thumbnail(version, _format="jpg"):
    ...
    for code in ["en", "tet", "pt", "id"]:
        ...
        if upload:
            ...
            if os.path.exists(
                os.path.join(
                    "/webapps/project/media/publication_covers", cover_file_name
                )
            ):
                setattr(
                    version,
                    "cover_%s" % (code),
                    "/webapps/project/media/publication_covers/%s" % (cover_file_name),
                )
                version.save()
                continue
            else:
                ...
                subprocess.call(
                    ["convert", upload.path + "[0]", _format + ":" + f.name]
                )
...
def import_sitrep():
    ...
    source_file = os.path.join(source_directory, "situation_review.csv")
    import_generic(
        source_directory, source_file, absolute_paths=False, master_publication=ewer_pub
    )
...
def import_trirep():
    ...
    import_generic(
        source_directory, source_file, absolute_paths=False, master_publication=p
    )
    ...
    def create_publication(r):
        ...
        if r.get("type", None):
            try:
                ...
                pub_kw["pubtype"] = Pubtype.objects.create(
                    name=r["type"], code=anti_vowel(r["type"]).upper()[0:3]
                )
        ...
        try:
            ...
            logging.error(
                "Invalid value for publication year of {}: ( {} )".format(
                    pub_kw["name"], r.get("year")
                )
            )
            ...
            for i in [i.strip() for i in r["country"].split(",")]:
                try:
                    ...
                    try:
                        ...
                        logging.error(
                            u"No Location: {} ( {} )".format(pub_kw["name"], i)
                        )
                    except AdminArea.MultipleObjectsReturned:
                        logging.warn(
                            u"Multiple Location: {} ( {} )".format(pub_kw["name"], i)
                        )
                        for adminarea in AdminArea.objects.filter(
                            name__iexact=i.strip()
                        ):
                ...
                try:
                    ...
                    try:
                        ...
                        if assists["location"].get(i):
                            publication.location.add(
                                AdminArea.objects.get(path=assists["location"].get(i))
                            )
                        else:
                            logging.error(
                                u"No Location: {} ( {} )".format(pub_kw["name"], i)
        ...
        if r.get("org", None):
            for i in [i.strip() for i in r["org"].split(",")]:
                ...
                try:
                    ...
                    publication.organization.add(
                        Organization.objects.create(name=i, orgtype_id="None")
                    )
...
def import_fm():
    ...
    try:
        ...
        pubtype = Pubtype.objects.get_or_create(
