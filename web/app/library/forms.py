...
class PublicationSearchForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            tag_id_choices = [
                ...
                for choice in Tag.objects.filter(
                    pk__in=[int(i) for i in self.data.getlist("tag__id")]
                )
            ]
        except:
            tag_id_choices = []
        ...
        self.fields["organization"] = forms.MultipleChoiceField(
            choices=Publication.objects.all()
        ...
        self.fields["year"] = forms.MultipleChoiceField(
            required=False,
            label=_("Year(s)"),
            choices=Publication.objects.order_by("year")
    ...
    primary = forms.BooleanField(required=False, label=_("Include primary sources"))
    ...
    language_id = forms.MultipleChoiceField(
        label=_("Language"), required=False, choices=LANGUAGES_FIX_ID
    )
    ...
    def helper(self):
        ...
        helper.layout = Layout(
            Fieldset(
                ...
                Field(
                    "tag__id",
                    wrapper_class=helper.wrapper_class,
                ...
                Field(
                    "year", wrapper_class=helper.wrapper_class, attrs={"maxItems": 2}
                ),
...
class PublicationAuthorForm(SuggestionForm):
    ...
    def __init__(self, publication=None, *args, **kwargs):
        super(PublicationAuthorForm, self).__init__(
            _instance=publication, *args, **kwargs
        )
        self.instance = publication
...
class VersionForm(SuggestionForm):
    ...
    def helper(self):
        ...
        helper.layout.extend(
            [
                TranslationTabs(
                    fieldnames=("title", "description", "upload", "cover", "url")
                ).tabholder
            ]
        )
