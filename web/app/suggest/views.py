...
def hide(request, model_name, model_pk, state="all"):
    ...
    l = Suggest.objects.filter(
        affectedinstance__model_name=model_name, affectedinstance__model_pk=model_pk
    )
    if state != "all":
...
def suggestlist(request, model_name=None, model_pk=None):
    ...
    context["filters"] = {
        "model_name": set(
            AffectedInstance.objects.all().values_list("model_name", flat=True)
        ),
    ...
    if model_name and model_pk:
        try:
            ...
            raise ValueError(
                "Expected an underscored app_project name eg nhdb_project -> ['nhdb', 'project']. Got %s"
                % g(model_name)
            )
    ...
    queryset = (
        Suggest.objects.filter(**filters)
        .distinct()
...
def suggestcreate(request):
    ...
    def get_primary_affected_instance(r):
        ...
        assert (
            i
        ), "There is no affected primary instance field on the submission - must have '_affected_instance_primary=nhdb project' or similar"

        assert (
            "_" in i
        ), 'Expected a string of the form "appname_modelname" but received {}'.format(
            i
        )
    ...
    for key, value in list(request.POST.items()):
        ...
        if key.startswith("__") or key in meta_fields:
            ...
            "_"
        ):  # Values with an underscore are from the metadata to be stored in Suggest table
            try:
    ...
    if warnings:
        return HttpResponse(
            content=json.dumps(warnings), content_type="application/json", status=400
        )
    ...
    if "_next" in request.POST:
        serialized_data["success_url"] = request.POST.get("_next").replace(
            "_suggestion_", str(s.id)
        )
