...
class SuggestTable(Table):
    ...
    def render_url(self, value, record):
        p = record.prepare
        cancelform = get_template("suggest/forms/cancel.html").render(
            {"record": record}
        )
        if not p["ready"]:
            f = "<p> Can't apply changes yet.<br> {} </p>".format(p["exception"])
            notreadyform = get_template("suggest/forms/not_ready.html").render(
                {"record": record}
            )
        ...
        if record.state == "W":
            if record.method == "DELETE":
                form = get_template("suggest/forms/delete.html").render(
                    {"record": record}
                )
            else:
                form = get_template("suggest/forms/main.html").render(
                    {"record": record}
                )
            ...
            return mark_safe(
                '<span style="color:{}">{}</span>'.format(
                    "green", record.get_state_display()
                )
            )
        elif record.state == "R":
            return mark_safe(
                '<span style="color:{}">{}</span>'.format(
                    "red", record.get_state_display()
                )
            )
        elif record.state == "X":
            return mark_safe(
