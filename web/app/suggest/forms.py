...
def choose_or_create(*args, **kwargs):
    ...
    layout.append(
        Button(
            "Add",
            "Add a new %s" % (fieldname),
            style="width:25%;",
            data_modalurl=data_modalurl,
        )
    )
    return layout
...
class DeleteFormHelper(FormHelper):
    def __init__(
        ...
        if not instance.pk:
            raise TypeError(
                "Cannot create a DeleteForm for an uninitialised object: {}".format(
                    instance
                )
            )
        ...
        if alert is True:
            try:
                self.layout.append(HTML(errormessage.format(instance)))
            except:
                self.layout.append(HTML(errormessage.format("an object")))
        ...
        try:
            ...
            if instance:
                ...
                description = kwargs.get(
                    "description", "Create a new {} in the database".format(name)
                )

                if instance.pk:
                    raise TypeError(
                        "Cannot create a CreateFormHelper for an exiting object"
                    )
        ...
        **kwargs
        ...
        try:
            ...
            if hasattr(instance, "_meta") and hasattr(instance, "pk"):
                ...
                name = instance._meta.verbose_name
                aip = Hidden(
                    "_affected_instance_primary", "{}_{} {}".format(app, mod, pk)
                )
                ...
                instance = "suggestion"
                aip = Hidden(
                    "_affected_instance_primary",
                    "{}_{} {}".format("suggest", "suggest", getattr(suggestion, "pk")),
                )
            ...
            if not description:
                description = kwargs.get(
                    "description",
                    "Modify a {} ({}) in the database".format(name, instance),
                )
    ...
    def __init__(self, suggestion=None, url=None, description=None, *args, **kwargs):
        try:
            ...
            if suggestion and not submit_url:
                ...
                if not submit_url:
                    submit_url = (
                        suggestion.url + "ERROR - might not be a primary instance?"
                    )
            ...
            self.layout = Layout(
                ...
                Hidden(
                    ...
                    or "Modify a {} in the database".format(name),
                ),
                Hidden(
                    "_affected_instance_primary",
                    "suggest_suggest {}".format(suggestion.pk),
                ),
                Hidden("_next", "/suggest/#object=_suggestion_"),
                manyfieldlayout(instance),
                Layout(*[Hidden("__nochange", f) for f in kwargs.get("nochange", [])]),
            ...
            self.layout.extend(
                [
                    HTML(
                        """<div class="alert alert-{}" role="alert">{}</div>""".format(
                            "warning", "UpdateSuggestionHelper"
                        )
                    )
                ]
            )
...
class SuggestDeleteForm(forms.ModelForm):
    ...
    def helper(self):
        helper = DeleteFormHelper(instance=self.instance)
        helper.layout.append(
            Hidden("__primary_key_field", self.instance.model._meta.pk.name)
        )
        return helper
...
class SuggestionForm(forms.ModelForm):
    def __init__(self, _instance, *args, **kwargs):
        ...
        if instance is None:
            ...
            raise TypeError(
                "_instance should be a %s or a Suggest object - got %s"
                % (self.Meta.model._meta.model_name, type(_instance))
            )
    ...
    def get_helper(self, **kwargs):
        ...
        if isinstance(self.suggest, Suggest):
            ...
            helper.layout.append(
                Layout(*[Hidden("__nochange", f) for f in self.nochange])
            ),
        ...
        helper.layout.append(
            Hidden("__primary_key_field", self.Meta.model._meta.pk.name)
        )
            ...
            if isinstance(instance, Suggest):
                ...
                try:
                    ...
                    n = (
                        ...
                        or getattr(self, _n, None)
                        or kwargs.get(_n, None)
                    )
                    ...
                    raise KeyError(
                        "%s not in fields: %s" % (_n, list(self.fields.keys()))
                    )

                except AttributeError as e:
                    raise AttributeError(
                        "There was a problem adding %s %s to the choices available"
                        % (_n, n)
                    )
