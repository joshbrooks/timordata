...
def _get_model(app_model_name):
    try:
        ...
        raise ValueError(
            "Looked for pattern (appname)_(modelname), did not find it in {}".format(
                app_model_name
            )
        )
...
class SuggestionManager(models.Manager):
    def suggest(self, obj):
        ...
        return self.filter(
            affectedinstance__model_name=model_name, affectedinstance__model_pk=obj.pk
        )
...
class AffectedInstance(models.Model):
    ...
    def __unicode__(self):
        ...
        return "From suggestion {} to change {} {} {}".format(
            self.suggestion.id, self.model_name, self.model_pk, self.primary
        )
    ...
    def retrieve_instance(self):
        ...
        try:
            ...
            raise model.DoesNotExist(
                "{}: query was .get(pk={})".format(e.message, self.model_pk)
            )
...
class SuggestUpload(models.Model):
    ...
    upload = models.FileField(
        upload_to="suggestions", max_length=256, null=True, blank=True
    )
...
class Suggest(models.Model):
    ...
    def __unicode__(self):
        ...
        if self.action == "CM":
            ...
            for p in parameter_try:
                if d.get(p):
                    return "(New %s) %s" % (
                        self.primary.model_name.split("_")[1],
                        d.get(p),
                    )
    ...
    method = models.CharField(
        max_length=128, choices=OPTIONS_METHOD, default="POST"
    )  # PUT POST PATCH DELETE - action expected by REST api
    data = models.TextField(
        null=True, blank=True
    )  # json REPR of suggested changes to the primary model
    suggestDate = models.DateField(auto_now_add=True)
    approvalDate = models.DateField(null=True, blank=True)
    url = models.CharField(
        max_length=128
    )  # Django-REST API URL to push the change to when it's approved
        ...
        def match(k, field):
            ...
            if re.match("_(\w+)_", k):
                ...
                try:
                    ...
                    logger.error(
                        "Tried to follow '%s' from '%s' with pk '%s'", key, to, k
                    )
                    logger.error(self.data_jsonify())
                    # raise
        ...
        if not m:
            ...
            for k, v in list(_d.items()):
                ...
                if t not in [
                    "ForeignKey",
                    "ManyToManyField",
                ]:  # Add other eligible FieldTypes here
                    continue
                ...
                if isinstance(related, Suggest) and (
                    related.state != "CM" or related.state != "A"
                ):
                    ready = False
                    continue
                ...
                # UNTESTED CODE!!!
                    ...
                    for index, i in enumerate(related):
                        ...
                        if isinstance(i, Suggest) and (
                            i.state != "CM" or i.state != "A"
                        ):
                            ready = False
                            continue
    ...
    def _url(self):
        ...
        r = {"ready": False, "url": "", "exception": None}
        ...
        try:
            ...
            r["ready"], r["url"], r["exception"] = (
                False,
                "",
                "Parent suggestion removed",
            )
            return r
        ...
        if related.primary.model_pk:
            ...
            r["ready"], r["url"], r["exception"] = (
                False,
                None,
                "Parent suggestion is not applied yet",
            )
        ...
        for attr, val in list(json.loads(str(self.data)).items()):
            ...
            if val is None:
                if self.action != "CM":
                    ...
                    changes.append(
                        (attr, current_value, "X", is_same(val, current_value))
                    )
                continue
            ...
            if self.action == "CM" and self.state == "W":
                ...
                try:
                    current_value = getattr(instance, attr)
                    changes.append(
                        (attr, current_value, val, is_same(val, current_value))
                    )
                except Exception as e:
        ...
        for u in self.suggestupload_set.all():
            changes.append(
                (
                    u.field_name,
                    "-",
                    mark_safe('<a href="%s">%s</a>' % (u.upload.url, u.upload.name)),
                    "-",
                )
            )
        ...
        if i.pk:
            ...
            for key, value_suggested in list(suggestion_data.items()):
                ...
                if get_field_type(i, key) == "ManyToManyField":
                    ...
                    value_current = set(
                        [str(_i) for _i in pks if not re.match("_(\w+)_", str(_i))]
                    )

                    # Get a set of suggested, updated, primary keys
                    value_new = set(
                        [v for v in value_suggested if not re.match("_(\w+)_", str(v))]
                    )
                    ...
                    current_objects = [
                        "%s" % (o) for o in f.objects.filter(pk__in=value_current)
                    ]
                    drop_objects = [
                        " - %s" % (o) for o in f.objects.filter(pk__in=drop)
                    ]
                    ...
                    add_objects.extend(
                        [" + %s" % (o) for o in _suggestions(value_suggested)]
                    )
                    r = ";".join(
                        [
                            wrap_class(keep_objects or "None unchanged", "unchanged"),
                            wrap_class(add_objects, "added"),
                            wrap_class(drop_objects, "removed"),
                        ]
                    )
                    returns[key] = r
                    ...
                    if value_current == value_suggested and value_suggested is not None:
                        ...
                        returns[key] = mark_safe("{} {}").format(
                            wrap_class(value_current, "removed"),
                ...
                # Try coercing to the same value
                    ...
                    if safe_compare(value_suggested, value_current):
                        ...
                        try:
                            returns[key] = mark_safe("{} {}").format(
                                wrap_class(unidecode(value_current or ""), "removed"),
            ...
            for key, value_suggested in list(suggestion_data.items()):
                if value_suggested is None:
                    ...
                    try:
                        returns[key] = wrap_class(
                            self.follow(key) or value_suggested, "unchanged"
                        )
    ...
    def references(self, fmt="flat"):
        ...
        if not hasattr(obj, "_meta"):
            raise TypeError(
                'No _meta for this object - ensure obj is a "model" instance'
            )
        ...
        for key in list(self.data_jsonify().keys()):
            ...
            if fmt == "list":
                ...
                raise TypeError(
                    'Invalid option for "fmt": try one of list, flat or dict'
                )
        return returns
    ...
    def parent(self):
        ...
        parent = Suggest.objects.filter(
            pk__in=[
                int(i)
                for i in self.affectedinstance_set.filter(
                    model_name="suggest_suggest"
                ).values_list("model_pk", flat=True)
            ]
        )
    ...
    def children(self):
        return Suggest.objects.filter(
            pk__in=[
                int(i)
                for i in AffectedInstance.objects.filter(
                    model_name="suggest_suggest", model_pk=self.pk
