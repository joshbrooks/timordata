...
def create_suggestion(data, code=201, client=None):
    ...
    if create_post.status_code != code:
        raise AssertionError(
            "Expected code %s, got code %s  /n Error message was %s"
            % (code, create_post.status_code, create_post.content)
        )
    ...
    for i in s.affectedinstance_set.all():
        logger.info(
            "%s -- %s -- %s -- %s"
            % (i.model_name, i.suggestion_id, i.model_pk, i.primary)
        )
...
def affirm(suggestion, code=201, client=None):
    ...
    r = c.post(
        suggestion.url,
        {"_content": suggestion.data, "_content_type": "application/json"},
    )
    ...
    if r.status_code != code:
        ...
        raise TypeError(
            "Wrong code received, expected {}; got {}".format(code, r.status_code)
        )
...
class NewProjectTestCase(TestCase):
    def setUp(self):
        ...
        s.affectedinstance_set.add(
            AffectedInstance(model_name="nhdb_project", model_pk=None, primary=True)
        )
        suggestion_pk = s.pk
        ...
        addOrg = Suggest.objects.create(
            ...
            data='{"project": "'
            + suggestion__pk
            + '", "organization": "89", "notes": "add Belun", "organizationclass": "P"}',
            url="/rest/nhdb/project/{}/".format(suggestion__pk),
        )
        addOrg.affectedinstance_set.add(
            AffectedInstance(
                model_name="nhdb_organization", model_pk=None, primary=True
            )
        )
    ...
    def test_suggestion_update(self):
        ...
        if not isinstance(test_form.get_helper(), forms.UpdateSuggestionHelper):
            msg = "Expected %s; got %s" % (
                "UpdateSuggestionHelper",
                type(test_form.get_helper()),
            )
            raise TypeError(msg)
    ...
    def test_suggestion_ManyToManyLink(self):
        ...
        def create_beneficiary():
            ...
            PropertyTag.objects.get_or_create(
                path="BEN", name="Beneficiary", description="Beneficiary"
            )
