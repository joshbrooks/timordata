...
class DonorSurveyResponse(models.Model):
    ...
    def __unicode__(self):
        return u"{} - {} - {}".format(
            self.organization_display, self.survey_date, self.get_response_display()
        )
...
class FundingSurvey(models.Model):
    ...
    organizationtype = models.ForeignKey(
        "nhdb.OrganizationClass", verbose_name=_("Organization type")
    )
    ...
    fundinggiven = models.NullBooleanField(
        default=None,
        verbose_name=_("Has your organization given funding in the past year?"),
    )
    fundinggiveamt = models.IntegerField(
        null=True,
        blank=True,
        verbose_name=_("How much (approximately) given in the past year?"),
    )

    fundingreceived = models.NullBooleanField(
        default=None,
        verbose_name=_("Has your organization received funding in the past year?"),
    )
    fundingrecvamt = models.IntegerField(
        null=True,
        blank=True,
    ...
    fundinggiverel = models.ManyToManyField(
        FundingSurveyInfoMethods,
        blank=True,
        related_name="fundinggiverel",
        verbose_name=_("How do you find your recipients?"),
    )
    fundinggivemethod = models.CharField(
        max_length=128, blank=True, verbose_name=_("If other, please write:")
    )

    fundingrecvrel = models.ManyToManyField(
        FundingSurveyInfoMethods,
        blank=True,
        related_name="fundingrecvrel",
        verbose_name=_("How do you find your donors?"),
    )
    fundingrecvmethod = models.CharField(
        max_length=128,
        null=True,
        blank=True,
        verbose_name=_("If other, please write:"),
    ...
    qfunding = models.IntegerField(
        null=True,
        blank=True,
        verbose_name=_("Donor funding is difficult to find"),
        choices=STATEMENTS,
    )
    qrecipients = models.IntegerField(
        null=True,
        blank=True,
        verbose_name=_("Recipient organisations are hard to find"),
        choices=STATEMENTS,
    )
    qtransport = models.IntegerField(
        null=True,
        blank=True,
        verbose_name=_("Receiving funds is difficult because of transport problems"),
        choices=STATEMENTS,
    )
    qprocess = models.IntegerField(
        null=True,
        blank=True,
        verbose_name=_(
            "Applying for funds is very difficult because the process is complicated"
        ),
        choices=STATEMENTS,
    )
    qdonorsector = models.IntegerField(
        null=True,
        blank=True,
        verbose_name=_(
            "Donor funding is hard to find for the sectors this organization works in"
        ),
        choices=STATEMENTS,
    )
    ...
    usefacebook = models.CharField(
        verbose_name=_("How often do you use Facebook?"),
        max_length=2,
        choices=FREQUENCIES,
    )
    usegmail = models.CharField(
        verbose_name=_("How often do you use Gmail?"), max_length=2, choices=FREQUENCIES
    )
    usegoogle = models.CharField(
        verbose_name=_("How often do you use Google Search?"),
        max_length=2,
        choices=FREQUENCIES,
    )
    ...
    hascomputer = models.BooleanField(
        default=False, verbose_name=_("I have access to a computer")
    )
    hasprinter = models.BooleanField(
        default=False, verbose_name=_("I have access to a printer")
    )
    hasmobile = models.BooleanField(
        default=False,
        verbose_name=_("I have access to a basic mobile phone (calls and SMS) at work"),
    )
    hassmartphone = models.BooleanField(
        default=False,
        verbose_name=_(
            "I have access to a smartphone or tablet (with internet access)"
        ),
    )
