...
class FundingOfferDocumentForm(SuggestionForm):
    ...
    def __init__(self, fundingofferdocument=None, fundingoffer=None, *args, **kwargs):
        ...
        self.fundingofferdocument, self.fundingoffer = fundingofferdocument, fundingoffer
    ...
    def helper(self):
        ...
        if self.fundingoffer:
            helper.layout.append(Field('offer', css_class="hidden", wrapper_class='hidden'))
        else:
    ...
    def helper(self):
        ...
        try:
            ...
            helper.layout.extend([
                Div(
                    Field('title'),
                    Field('organization', data_selecturl='/selecttwo/nhdb/organization/name/icontains', **create_organization_elements),
                    Field('amount'),
...
class FundingOfferSearchForm(forms.Form):
    ...
    try:
        district = forms.MultipleChoiceField(
            choices=District.objects.all().values_list('pk', 'name').distinct().order_by('name'),
            label=_("District")
        )
    except:
        pass
